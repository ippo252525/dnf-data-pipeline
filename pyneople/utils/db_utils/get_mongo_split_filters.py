...
def get_split_filters_for_task(num_workers: int = 6):
    """
    MongoDB _id를 기준으로 num_workers개 범위 필터를 계산해서 반환하는 비동기 함수
    ...
    mongo_collection = db[Settings.MONGO_COLLECTION_NAME]        
    
    total_docs = mongo_collection.estimated_document_count()
    split_points = []

    for i in range(1, num_workers):
        skip = int(total_docs * i / num_workers)
        doc = mongo_collection.find({}).sort('_id', 1).skip(skip).limit(1).to_list(1)
        if doc:
            split_points.append(doc[0]['_id'])
